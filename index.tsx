...
function CursorShadowTextHero(props: HeroComponentProps & Record<string, any>) {
	const { rootRef, reducedMotion, pointer,
		heading = 'follow the light',
		shadowColor = '#5b8def', maxOffset = 40, blur = 12,
		textColor = '#ffffff', fontSize = 160,
		headingFontFamily = 'Inter', headingFontWeight = '800',
	...
	return (
		<>
			...
			<crazygl-content>
				<div ref={hRef} className="crazygl-cst-h" style={{ fontFamily: ff, fontWeight: weight, fontSize: `${Math.max(28, fontSize)}px`, color: textColor }}>{heading}</div>
			</crazygl-content>
		</>
	);
}

export default function CursorShadowText(props: any) { return <CrazyGLWrapper hero={CursorShadowTextHero} metadata={metadata as any} {...props} />; }
export { metadata };
